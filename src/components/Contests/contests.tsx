...
const PLATFORM_META: Record<string, { label: string; badge: string; dot: string }> = {
  leetcode:    { label: "LeetCode",    badge: "bg-yellow-500/10 text-yellow-400 border-yellow-500/30",  dot: "bg-yellow-400"  },
  codeforces:  { label: "Codeforces",  badge: "bg-blue-500/10 text-blue-400 border-blue-500/30",        dot: "bg-blue-400"    },
  codechef:    { label: "CodeChef",    badge: "bg-orange-500/10 text-orange-400 border-orange-500/30",  dot: "bg-orange-400"  },
  atcoder:     { label: "AtCoder",     badge: "bg-teal-500/10 text-teal-400 border-teal-500/30",        dot: "bg-teal-400"    },
  hackerearth: { label: "HackerEarth", badge: "bg-indigo-500/10 text-indigo-400 border-indigo-500/30", dot: "bg-indigo-400"  },
  csacademy:   { label: "CS Academy",  badge: "bg-red-500/10 text-red-400 border-red-500/30",           dot: "bg-red-400"     },
  code360:     { label: "Code360",     badge: "bg-rose-500/10 text-rose-400 border-rose-500/30",        dot: "bg-rose-400"    },
};
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <span className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-[0.7rem] font-semibold tracking-wide ${meta.badge}`}>
          <span className={`h-1.5 w-1.5 rounded-full ${meta.dot}`} />
          {meta.label}
        </span>

        <span className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-[0.68rem] font-bold uppercase tracking-widest ${statusBadge}`}>
          {status === "live" && (
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-12 text-center">
          ...
          <h2 className="text-4xl font-bold leading-tight tracking-tight text-white md:text-5xl">
            Upcoming{" "}
            <span className="text-[#8C45FF]">Contests</span>
          </h2>
        ...
        <div className="mb-10 flex flex-wrap justify-center gap-2">
          {platforms.map((p) => {
            ...
            const meta = p === "all"
              ? { label: "All Platforms", badge: "bg-purple-500/10 text-purple-400 border-purple-500/30" }
              : getPlatform(p);
            ...
            return (
              <button
                ...
                className={`cursor-pointer rounded-full border px-4 py-1 text-xs font-medium transition-all duration-200 ${
                  isActive
                    ? meta.badge
                    : "border-white/15 text-white/40 hover:border-white/30 hover:text-white/60"
                }`}
